...
export async function generateInvoicePDF(data: InvoiceData): Promise<void> {
  ...
  if (template.id === 'minimalist') {
    ...
    doc.text(sanitizeText(data.companyInfo.name), margin, currentY);
    ...
    doc.setFontSize(24);
    doc.text(sanitizeText(data.companyInfo.name), margin, currentY + 5);
  ...
  // Add company details from form data
  if (data.companyInfo.address) {
    doc.text(sanitizeText(data.companyInfo.address), margin, currentY);
    currentY += 5;
  }
  ...
  const cityStateZip = [
    ...
    data.companyInfo.state,
    data.companyInfo.zipCode
  ].filter(Boolean).join(', ');
  
  if (cityStateZip) {
    doc.text(sanitizeText(cityStateZip), margin, currentY);
    currentY += 5;
  }
  
  if (data.companyInfo.phone) {
    doc.text(`Phone: ${sanitizeText(data.companyInfo.phone)}`, margin, currentY);
    currentY += 5;
  }
  
  if (data.companyInfo.email) {
    doc.text(`Email: ${sanitizeText(data.companyInfo.email)}`, margin, currentY);
    currentY += 5;
  }
